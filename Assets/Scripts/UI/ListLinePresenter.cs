...
{
    /// Yarn Spinner's LinePresenter.cs EXCEPT it shows every new line in a new
    /// text box.

    public sealed class ListLinePresenter : DialoguePresenterBase
    {
        /// <summary>
        ...
        private void Awake()
        {
            ...
            foreach (TMP_Text? text in lineTexts)
            {
                text.text = "";
            }
        }
        ...
        {
            if (lineTexts.Length == 0 || linesDisplayed >= lineTexts.Length || lineTexts[linesDisplayed] == null)
            {
                Debug.LogError($"Line view does not have a text view. Skipping line {line.TextID} (\"{line.RawText}\")");
                return;
            }

            TMP_Text? lineText = lineTexts[linesDisplayed];
            ...
            if (useTypewriterEffect)
            {
                var typewriter = new BasicTypewriter()
                {
                    ...
                    Text = this.lineTexts[linesDisplayed],
            ...
            linesDisplayed = (linesDisplayed + 1) % lineTexts.Length;
        }
